...
| `const_splat!` / `const_new!` (`vector/splat.rs`) | const expr | Compile-time splat / per-lane const vector. **Use for bitwise/coefficient constants** (`const_splat!(u32: 0x5555_5555)`), never a bare `const`. |
