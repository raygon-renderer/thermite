...
> SIMD is a lot like async programming. Go at it like you would ordinary
> sync or scalar code and it will probably be a pretty bad experience.
> Thermite tries to help as much as it possibly can, but some of the onus is on you to
> understand what the machine is actually doing, and this guide is paced around building
> that understanding.

## Introduction

Thermite's goal is to let you write simple, high-performance code that is generic both
across hardware backends and across vector widths. The same code can be invoked with
`f32x8` on AVX2 (x86-v3) or with `f64x2` on ARM NEON.
...
only the capability the body needs. `#[thermite::dispatch]` is what keeps it _fast_: it
emits one `#[target_feature]`-annotated copy of the body per backend, which is the only
way the intrinsics behind `exp` can be inlined at all. If `#[thermite::dispatch]` isn't
included, Rust will "forget" which backend this is compiling for, and de-optimize the
entire thing. Seriously: skipping it is worse than not using SIMD at all, and the
Performance section shows the exact assembly you get without it. If you're skeptical of
Rule 1, jump there first.
...
vector, and `f32xN` is its _widest native_ one (4 lanes on SSE2 or NEON, 8 on AVX2, 16
on AVX-512). Ask the type how wide it turned out to be rather than hard-coding it.

The loop below is deliberately the manual version, chunking, tail handling, and all.
This is the true shape of SIMD work: data comes in register-sized pieces, something has
to deal with the ragged edge, and the scalar path doubles as the ground truth for the
vectorized one. `SimdSlice` (covered later) automates all of it, but it's automating
exactly this, so it's worth seeing spelled out once:
...
Five of the feature flags matter in practice:
...
One more needs a warning, not a description. **`algebraic-scalar`** lets LLVM
...
That runs on the scalar backend, one lane wide, so it needs no dispatch at all.
...
the last one. The registers are low-level, backend-specific types wrapping the CPU's raw
SIMD registers, and they are what Rule 0 is about. The `Vector` traits are the high-level
side: the same operations with the same semantics, no matter which backend sits
underneath. Write against the traits and the code is portable across every backend for
free.

That said, knowing the parameters of the backend you landed on is genuinely useful for
tuning. The lane count alone decides how many elements each loop iteration eats, and
later sections lean on it constantly.

Every type implementing `GenericVector` also requires `HasIsa`, an escape hatch of sorts
to some of the native hardware's capabilities, and any custom type can `#[derive(HasIsa)]`
to forward that information. `#[thermite::dispatch(T)]` requires that `T` implements
`HasIsa`, which is how it knows what backend to dispatch to, and `dispatch_dyn!` can
inject such a type into its pseudo-closure so it can be passed down.

The traits are also the entire contract. A vector doesn't have to be anything more than
what the traits require, and Thermite leans on that hard: composite types like `Complex`,
`Dual`, and `Compensated` implement the same traits, which means _any generic function
accepting some vector type will also accept them_. Entire codebases can be written in
terms of generic vector types and later be run with `Dual` or `Compensated` to get
forward-mode automatic differentiation (`thermite-dual` ships an `AutoDiff` trait that
makes this a one-liner) or higher precision, with zero changes to the original code.

Composites even nest, within limits: `Complex<Compensated<V>>` is complex arithmetic in
double-double precision, and `Dual<Compensated<V>, N>` differentiates in it. That last
combination earns its keep where `f64` isn't available at all, like GPUs, where
`Compensated` claws back near-double accuracy from single-precision hardware. Absolutely
wild, right? Even `thermite-sort` works with `Dual` and `Compensated` unchanged, so
sorting double-doubles or complex numbers comes free (in their in-memory SoA forms, at
least).
...
That uniformity is on purpose. Code that wants sixteen lanes of `f32` can ask for sixteen
lanes of `f32` and it will compile and run correctly on a machine whose registers hold
four. However, it cannot make that free.
...
reason: on a GPU the parallelism comes from running many invocations instead of wide
registers, so the width lives in the dispatch instead of in the type.

### Small elements get very wide

Look at the right-hand columns above. A register is a fixed number of _bits_, so the
...
There is a catch, because the fixed-width names stop at `x16` for every element family.
There is no `u8x32`, and there is no `u8x64`. Those registers are
...
So the rule from the previous section applies with more force here, not less. Byte and
short work, which is most classification, text processing, and image work, is exactly
where `xN` pays the most: reach for it by default and let the backend decide how wide
that is.
...
There is one exception. For the smallest element types, a two-lane vector is
literally two scalars, not a masked-off register, because at that size there's
...
The `x3A` types are three lanes stored in a four-lane register, with the fourth left
...
That has a consequence which is easy to trip over, so it is spelled out here.
...
The byte footprint is what stays outside the contract. So don't compute offsets
...
The default is not a prohibition. The opposite mistake also exists. This is the other half
of Rule 2: commit to a fixed lane count when the data structure is genuinely defined by it.
...
even on a machine whose registers don't match. A structure whose properties you picked
on purpose is worth more than the throughput you would claw back by letting the width
...
a genuine mistake is a _generic_ function that inspects `V::LANES` and changes its algorithm
...
Past `dispatch_dyn!` and `#[dispatch]`, the entry points are the `Vector` traits
themselves, and the important ones form a short ladder:

- `GenericVector` is the base of everything. It assumes nothing beyond a sequence of
  lanes, without any particular arithmetic.
- `NumericVector` adds arithmetic operations, but not necessarily floating-point ones.
- `FloatVector` adds the floating-point operations (the transcendentals come from the
  math traits, covered below).
- `IntegerVector`, with `SignedIntegerVector` and `UnsignedIntegerVector`, covers the
  integer side.
...
That's a multiply followed by an add. Fused multiply-add exists for exactly this.
Reaching for `mul_adde` costs one more bound, because `MulAddExt` is a supertrait of
`FloatVector` and not `NumericVector`:
...
without it. That trade is the entire skill, so make it knowingly.

### Check the surface before you write anything

Rule 4 is worth repeating here, because the trait surface is much larger than a first read suggests.
...
prelude imports the math traits _anonymously_, so their methods resolve but the names
...
though, reach for `scale` instead of splatting and multiplying by hand. It carries the
...
Branching on `V::LANES` is a subtler version of the same mistake. If two widths really do
...
Note the difference between that and choosing a fixed width on purpose, covered under
vector widths above. A structure that _is_ eight wide should say so in its types. A generic
function that asks how wide it happens to be, and does something else when the
answer changes, is the problem.

## Reductions

`sum_elements`, `prod_elements`, `min_element`, and `max_element` collapse a vector into
one scalar. Each costs `O(log2 LANES)` shuffle-and-combine steps, which makes them the
most expensive direction to move data: across the lanes, then out of the register
entirely. That cost is fine paid once. It is a disaster paid every iteration.

So the shape of a reduction loop is always the same. Accumulate _vertically_, lane
against lane, for the whole loop, and reduce _horizontally_ exactly once at the end. The
`dot2` kernel above already computed per-lane products. A full dot product over two
buffers is that, plus an accumulator, plus one final reduce:

```rust
use thermite::prelude::*;

// 103 elements: odd, so the scalar tail is real.
let xs: Vec<f32> = (0..103).map(|i| i as f32 * 0.01).collect();
let ys: Vec<f32> = (0..103).map(|i| (103 - i) as f32 * 0.01).collect();

let dot = thermite::dispatch_dyn!(|xs: &[f32], ys: &[f32]| -> f32 {
    let x_chunks = xs.chunks_exact(f32xN::lanes());
    let y_chunks = ys.chunks_exact(f32xN::lanes());
    let (x_tail, y_tail) = (x_chunks.remainder(), y_chunks.remainder());

    // Vertical: each iteration is one FMA, and no lane ever talks to another.
    let mut acc = f32xN::ZERO;
    for (x, y) in Iterator::zip(x_chunks, y_chunks) {
        acc = f32xN::from_slice(x).mul_adde(f32xN::from_slice(y), acc);
    }

    // Horizontal: the shuffle cascade happens exactly once.
    let mut dot = acc.sum_elements();

    for (&x, &y) in Iterator::zip(x_tail.iter(), y_tail.iter()) {
        dot += x * y;
    }
...
- A single accumulator is a single dependency chain. Each FMA waits four to five cycles
  on the previous one while the core could be starting two per cycle, so a loop this
  small runs at a fraction of peak. Splitting into two or four accumulators and
  combining them just before the final reduce lets iterations overlap. The
  dependency-chain discussion in the performance section is exactly this trade.
- Vertical accumulation adds in a different order than a left-to-right scalar loop, so
  the result is not bit-identical to one (it is usually _closer_ to the true sum, since
  each lane's partial total stays smaller). When the rounding matters, that is
  what `Compensated` is for.

One scope note: these reduce a batch. If the vector holds a single object, one ray, one
color, one point, then the `3`-suffixed forms under the 3D and 4D primitives are the
right tool, not these.
...
single register. It does mean a piecewise function costs the sum of its pieces, not
the larger one, so check whether some single formulation covers both cases first.
...
Masks answer a few more questions. `first_set` and `last_set` give the
...
Everything in this section consumes the masks you just made, and none of it is obvious
coming from scalar code. Three primitives come up constantly once you're past simple
element-wise work.

**Compaction.** `compress` left-packs the lanes a mask selects, so `[a, b, c, d]` with
`[true, false, true, false]` becomes `[a, c, b, d]`, selected lanes first. Store just the
leading `count_set` lanes afterward and you have stream compaction, which is how you
filter, strip, or minify without branching. `compress_z` zeroes the tail instead
...
`[1, 3, 6, 10]`. That is how you turn per-lane counts into write offsets. There are
...
**Divergence.** Read this one even if you never use it. A packet whose
...
Policies are types, not values, and their parameters are associated constants, so
...
which is the right call on both, and better learned here than by surprise.
...
That last parameter matters much more than its name suggests. Denormals can
...
and promised that `SimdSlice` automates the whole dance. Here it is. It's implemented
for `[T]`, so it's available on anything that derefs to a slice:
...
Reach for `try_aligned_simd_iter` by default. It never panics, it hands back
...
already exactly aligned with no remainder on either end. It panics instead of coping,
which is the point: it turns a layout assumption into a loud failure instead of a slow
path you never notice.

`unaligned_simd_iter` handles arbitrary slices, and yields vectors by value instead of by
...
returns a pair and not a triple, just the iterator and a trailing remainder, because
...
Alignment matters in hot loops over large buffers and nowhere else. `V::align_slice` is the
underlying split if you want it directly. One detail: for a vector whose byte size isn't
its element size times its lane count, which happens with a non-hardware width like a
3-lane vector, `align_slice` gives up and returns the whole slice as unaligned rather than pretending.
...
to forget given that you reached for them for the size of the buffer and not
its layout. The streaming slice iterators enforce it for you by panicking on a misaligned or
ragged slice, so you find out early.
...
`scatter` writes back the same way. Indices are counted in elements, not bytes. The
...
Reach for these when the access pattern really is irregular, such as walking an index
...
of range indices yield the table's first element instead of panicking. If you're indexing
a small constant table, which covers most polynomial coefficient selection and most
classification work, this is what you want.
...
and that is a bad idea for now. Pointing it past the end of an allocation and trusting the
...
gives you `[0, 1, 2, ...]`, the usual starting point for an index-carrying loop
...
Watch out, because the two spell things similarly. A lane-wise SoA dot product over separate
...
When you do need a clean fourth lane, which homogeneous coordinates require, `zero4`
...
The `mat3_*` and `mat4_*` families take `COLUMN_MAJOR`, and this one is nastier than it seems.
It selects storage convention, which is fine, but for `mat3_product` and
...
For `quat_to_mat3` and `quat_to_mat4` the same parameter is actually free, since only the
...
Read the matrix inverses before using them. `mat3_inverse_inplace` **returns the
determinant** and not the matrix, avoids a copy, and lets you apply your own tolerance.
An exactly zero determinant leaves the matrix untouched, and a near-zero one gives you a
finite but unreliable result, so inspecting the returned value is the entire point of the variant.
...
costs as much and nothing else fails as invisibly.
...
### `#[dispatch]` is also how you stop inlining

That last point is worth turning around, because it is the half of Rule 1 that gets
missed. A dispatched function behaves like a plain `#[inline]` one at its own boundary,
the compiler decides whether to fold it into the caller, exactly as it would for any
ordinary function. It never loses the target features, which stay enabled
throughout the body regardless of that decision.

That makes `#[dispatch]` the right attribute for a real API entry point, not only for the
innermost hot kernel. Annotate the public function, keep everything beneath it
`#[inline(always)]`, and the interior still collapses into one feature-enabled body while
the entry point stays something the compiler can size on its own.

You can also force the boundary, and this needs no special support. `#[inline(never)]` and
`#[cold]` are ordinary Rust attributes behaving the way they always do, and `#[dispatch]`
does not interfere with either, so you get a guaranteed function call with the target
features intact. `#[cold]` is worth remembering for fallback and error paths that would
otherwise bloat a hot caller.

Without any of this, a large codebase turns into one undifferentiated `#[inline(always)]`
soup: compile times climb, the optimizer chews on enormous bodies, and there are no
boundaries left where a copy can be shared. `#[dispatch]` gives the structure back. Put it
where a module's real seams are, and reach for `#[inline(always)]` beneath them.
...
negation into the constant and use `mul_adde` instead of `nmul_adde`, which saves an
...
put their bodies outside the feature context. That is the Rule 1 failure one level down.
...
costs much more than the compare itself, because it breaks LLVM's block scheduling around
...
command. For timing, use a real harness, not eyeballing, and remember that a debug
build of SIMD code is slow by construction, so measure in release.

And that's really what Rule 6 is about. Doing nothing is better than doing something
clever: the plain spelling of a kernel usually schedules well, and every trick in this
section is only worth keeping if the assembly or the benchmark says so. Write the obvious
version, look at what you got, and only then get clever.

## Composite Types

Because the trait bounds are the entire contract, a type that is not a hardware register
at all can implement the same hierarchy and flow through the functions
you already wrote. Nothing in `dot2` or `decay` from earlier needs to change.
...
And a few things that are not traps, just easy to miss:

**`#[dispatch]` isn't only for functions.** It also goes on an `impl` block, where every
method gets wrapped and `Self` resolves correctly, on a trait definition, or on a whole
`mod`, which applies it recursively to every `fn` and `impl` inside. Mark individual
items `#[skip_dispatch]` to opt them out.

**fp16, bf16, and fp8 exist as storage formats.** `PackedFloatVector` treats a `u16` or
`u8` vector as packed floats and transcodes to and from a full `f32` vector, so memory
holds half or a quarter of the bytes while the arithmetic runs at full precision. The
`avx2-f16c` default makes the fp16 leg a hardware conversion on x86.

**Morton codes are built in.** `morton::<N>` interleaves the bits of `N` vectors into
per-lane Z-order indices, and `reverse_morton` splits them back apart. Sorting by the
result groups spatially near points together, and the `avx2-pclmul` default gives the 2D
form a carry-less-multiply fast path on `u64` lanes.

**Polynomial evaluation is built in.** `poly` and `poly_rev` evaluate a const-sized
coefficient array in either order, fusing with FMA where the hardware has it, and
`poly_rational` computes a ratio of two polynomials with better stability in some cases
...
by falling back to a software pack. Both directions work: `from_native_bitmask` rebuilds
a mask from a `u64` (a broadcast, AND, and compare on most backends, a single `vpmovm2*`
on AVX-512), and `from_bitmask` does the same from a `bitvec` slice at any width. This
is the bridge between SIMD comparisons and scalar-side index work.
