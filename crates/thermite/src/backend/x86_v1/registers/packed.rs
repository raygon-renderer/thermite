//! `PackedFloatRegister` for the native 16-bit registers on x86-v1.
//!
//! x86-v1 (SSE2) has no F16C, so every 16-bit-float format uses the generic branchless
...
use super::half16::U16x4V1;
use super::{F32x4V1, U16x8V1};

/// f32 partner per `u16` width - the backend's `Simd::f32xK` (native f32 is 4-wide on v1, so the
...
impl_packed_f16!(U16x4V1, F32x4);
impl_packed_f16!(U16x8V1, F32x8);
impl_packed_f16!(ArrayRegister<U16x8V1, 2>, F32x16);
