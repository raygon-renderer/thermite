...
impl Register for I32x8V3 {
    ...
    unsafe fn store_masked(ptr: *mut Self::Element, mask: Storage<Self::Mask>, value: Storage<Self>) {
        unsafe { arch::_mm256_maskstore_epi32(ptr, mask, value) }
    }
    ...
    // A 32-bit-element register: `(8, 1)` is the 8x8 (W=4 bytes) transpose, `(4, 2)` the 4x4
    // (W=8 bytes) one - both bit-cast into the shared ps/pd sequences (free casts).
    ...
    fn interleave_radix_by<const N: usize, const GROUP: usize>(inputs: [Storage<Self>; N]) -> [Storage<Self>; N] {
        // Both transposes are self-inverse, so interleave reuses the same bodies.
        if const { N == 8 && GROUP == 1 } {
            unsafe { arch::radix_by_w32_si(inputs) }
        } else if const { N == 4 && GROUP == 2 } {
            unsafe { arch::radix_by_w64_si(inputs) }
