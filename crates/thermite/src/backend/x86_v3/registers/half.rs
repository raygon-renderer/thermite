...
// truncating narrow. The `ArrayRegister`-SOURCE cousins live on their source registers
// (`i64x4.rs` / `u64x4.rs`); the `ArrayRegister`-DESTINATION ones are generated by `impl_casts!`
// in `register/array.rs` once these native bases exist.
macro_rules! sat_gap {
    ($from:ty, $fe:ty, $ie:ty) => {
        #[inline(always)]
...
// `vpmovqd`-less 64 -> 32 narrows. `impl_type_casts!` cannot stamp these because they
// need both strengths, so they are written out here instead of in that table.
#[thermite_macros::inline_always]
impl CastRegister<super::I64x4V3> for super::I32x4V3 {
    fn cast_from(value: Storage<super::I64x4V3>) -> Storage<Self> {
        unsafe { arch::_mm256_cvtepi64_epi32_v3(value) }
    }

    sat_gap!(super::I64x4V3, i64, i32);
}

#[thermite_macros::inline_always]
impl CastRegister<super::U64x4V3> for super::U32x4V3 {
    fn cast_from(value: Storage<super::U64x4V3>) -> Storage<Self> {
        unsafe { arch::_mm256_cvtepi64_epi32_v3(value) }
    }

    sat_gap!(super::U64x4V3, u64, u32);
}
