...
pub fn convert_pd_epi64_limited<R: FloatRegister<Element = f64>>(x: Storage<R>) -> Storage<R::Signed> {
    ...
    <R::Signed as BitCastRegister<R>>::from_bits(R::bitxor(R::add(x, m), m))
}
...
pub fn convert_epi64_pd_limited<R: FloatRegister<Element = f64>>(mut x: Storage<R::Signed>) -> Storage<R> {
    ...
    x = <R::Signed as NumericRegister>::add(x, <R::Signed as BitCastRegister<R>>::from_bits(m));

    R::sub(<R as BitCastRegister<R::Signed>>::from_bits(x), m) // _mm_sub_pd(_mm_castsi128_pd(...), m)
}
