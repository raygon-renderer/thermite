//! `f32x8` on AVX-512: the 256-bit float register under EVEX (AVX512VL).
//!
//! Same storage type as the v3 `f32x8`, nothing else in common: opmask
//! compares, embedded-mask ops, native `vpternlog`/`vpermt2ps`/compress and
//! every `_c`/`_m`/`_z` variant written out, exactly as in the 512-bit
//! pattern file ([`f32x16`](super::f32x16), read its module docs for the
//! house rules). Differences that are purely width:
//!
//! - `KMask8` masks. `__mmask8` constants for `zeroupper_z`.
//! - No `_mm256_reduce_*` sequences exist in `core::arch`, so the element
//!   reductions use the v3 `_mm256_reduce_ps_v3!` fold (AVX, same width -
//!   not a half-split).
//! - `ExtendedPrecision` is the NATIVE `f64x8` (`__m512d`): eight lanes of
//!   f32 widen to one zmm of f64, so the widen/narrow legs are the single
//!   `vcvtps2pd`/`vcvtpd2ps` zmm forms.
//! - 64-bit-index gathers take the eight indices in one zmm (`u64x8`).
...
impl CoreRegister for F32x8V4 {
    ...
    fn blendv(mask: Storage<Self::Mask>, on_false: Storage<Self>, on_true: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_blend_ps(mask, on_false, on_true) }
    }

    fn zz(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_mov_ps(mask, value) }
    }
    ...
    fn zeroupper_z<Z: ZeroUpper>(value: Storage<Self>) -> Storage<Self> {
        if const { Z::N >= 8 } {
            ...
            unsafe { arch::_mm256_maskz_mov_ps(const { ((1u32 << Z::N) - 1) as u8 }, value) }
        }
    }

    fn from_mask(mask: Storage<Self::Mask>) -> Storage<Self> {
        unsafe { arch::_mm256_castsi256_ps(arch::_mm256_movm_epi32(mask)) }
    }
}

#[rustfmt::skip] #[thermite_macros::inline_always]
impl BitwiseRegister for F32x8V4 {
    ...
    fn bitandnot(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_andnot_ps(rhs, lhs) }
    }
    ...
    fn not(value: Storage<Self>) -> Storage<Self> {
        ...
        unsafe {
            arch::_mm256_castsi256_ps(arch::_mm256_ternarylogic_epi32::<{ crate::ternlog_imm!(!C) }>(
    ...
    fn ternlog<const IMM: i32>(a: Storage<Self>, b: Storage<Self>, c: Storage<Self>) -> Storage<Self> {
        unsafe {
            arch::_mm256_castsi256_ps(arch::_mm256_ternarylogic_epi32::<IMM>(
                arch::_mm256_castps_si256(a),
                arch::_mm256_castps_si256(b),
                arch::_mm256_castps_si256(c),
            ))
        }
    }
    ...
    masked_binary_v4! {
        bitxor => _mm256_mask_xor_ps, _mm256_maskz_xor_ps;
        bitand => _mm256_mask_and_ps, _mm256_maskz_and_ps;
        bitor => _mm256_mask_or_ps, _mm256_maskz_or_ps;
    }

    masked_andnot_v4! {
        bitandnot => _mm256_mask_andnot_ps, _mm256_maskz_andnot_ps;
    }
    ...
    fn not_c(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        let v = unsafe { arch::_mm256_castps_si256(value) };
        unsafe { arch::_mm256_castsi256_ps(arch::_mm256_mask_ternarylogic_epi32::<{ crate::ternlog_imm!(!C) }>(v, mask, v, v)) }
    }

    fn not_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        let v = unsafe { arch::_mm256_castps_si256(value) };
        unsafe { arch::_mm256_castsi256_ps(arch::_mm256_mask_ternarylogic_epi32::<{ crate::ternlog_imm!(!C) }>(arch::_mm256_castps_si256(src), mask, v, v)) }
    }

    fn not_z(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        let v = unsafe { arch::_mm256_castps_si256(value) };
        unsafe { arch::_mm256_castsi256_ps(arch::_mm256_maskz_ternarylogic_epi32::<{ crate::ternlog_imm!(!C) }>(mask, v, v, v)) }
    }

    fn ternlog_c<const IMM: i32>(mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>, c: Storage<Self>) -> Storage<Self> {
        unsafe {
            arch::_mm256_castsi256_ps(arch::_mm256_mask_ternarylogic_epi32::<IMM>(
                arch::_mm256_castps_si256(a),
                mask,
                arch::_mm256_castps_si256(b),
                arch::_mm256_castps_si256(c),
    ...
    fn ternlog_m<const IMM: i32>(src: Storage<Self>, mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>, c: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_mov_ps(src, mask, Self::ternlog::<IMM>(a, b, c)) }
    }

    fn ternlog_z<const IMM: i32>(mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>, c: Storage<Self>) -> Storage<Self> {
        unsafe {
            arch::_mm256_castsi256_ps(arch::_mm256_maskz_ternarylogic_epi32::<IMM>(
                mask,
                arch::_mm256_castps_si256(a),
                arch::_mm256_castps_si256(b),
                arch::_mm256_castps_si256(c),
...
#[thermite_macros::inline_always]
impl crate::register::ConcatRegister<super::F32x4V4> for F32x8V4 {
    fn concat(lo: Storage<super::F32x4V4>, hi: Storage<super::F32x4V4>) -> Storage<Self> {
        unsafe { arch::_mm256_setr_m128(lo, hi) }
    }

    fn split(value: Storage<Self>) -> (Storage<super::F32x4V4>, Storage<super::F32x4V4>) {
        let lo = unsafe { arch::_mm256_castps256_ps128(value) };
        let hi = unsafe { arch::_mm256_extractf128_ps::<1>(value) };
        (lo, hi)
    }
}

#[thermite_macros::inline_always]
impl crate::register::ExtendRegister<super::F32x4V4> for F32x8V4 {
    fn extend(value: Storage<super::F32x4V4>) -> Storage<Self> {
        unsafe { arch::_mm256_zextps128_ps256(value) }
    }

    fn narrow(value: Storage<Self>) -> Storage<super::F32x4V4> {
        unsafe { arch::_mm256_castps256_ps128(value) }
    }
}

#[thermite_macros::inline_always]
impl crate::register::WideRegister for F32x8V4 {
    type Wide = super::F32x16V4;
}

#[thermite_macros::inline_always]
impl Register for F32x8V4 {
    type Element = f32;

    type Signed = super::I32x8V4;
    type Unsigned = super::U32x8V4;

    fn into_mask(value: Storage<Self>) -> Storage<Self::Mask> {
        unsafe { arch::_mm256_test_epi32_mask(arch::_mm256_castps_si256(value), arch::_mm256_castps_si256(value)) }
    }
    ...
    fn msb_to_mask(value: Storage<Self>) -> Storage<Self::Mask> {
        unsafe { arch::_mm256_movepi32_mask(arch::_mm256_castps_si256(value)) }
    }
    ...
    fn single(value: Self::Element) -> Storage<Self> {
        unsafe { arch::_mm256_zextps128_ps256(arch::_mm_set_ss(value)) }
    }
    ...
    unsafe fn load_m(src: Storage<Self>, mask: Storage<Self::Mask>, ptr: *const Self::Element) -> Storage<Self> {
        unsafe { arch::_mm256_mask_loadu_ps(src, mask, ptr) }
    }

    unsafe fn load_z(mask: Storage<Self::Mask>, ptr: *const Self::Element) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_loadu_ps(mask, ptr) }
    }
    ...
    unsafe fn store_masked(ptr: *mut Self::Element, mask: Storage<Self::Mask>, value: Storage<Self>) {
        unsafe { arch::_mm256_mask_storeu_ps(ptr, mask, value) }
    }
    ...
    unsafe fn lookup(values: &[Self::Element], indices: Storage<Self::Unsigned>) -> Storage<Self> {
        if values.len() <= <Self::Lanes as Unsigned>::USIZE {
            ...
            unsafe { arch::_mm256_permutexvar_ps(indices, Self::new(padded)) }
        } else {
    ...
    fn reverse(value: Storage<Self>) -> Storage<Self> {
        unsafe {
            ...
            arch::_mm256_permutexvar_ps(idx, value)
        }
    }
    ...
    fn swap_bytes(value: Storage<Self>) -> Storage<Self> {
        unsafe {
            ...
            arch::_mm256_castsi256_ps(arch::_mm256_shuffle_epi8(arch::_mm256_castps_si256(value), pattern))
        }
    }
    ...
    fn broadcast_m<const I: usize>(
        ...
        unsafe { arch::_mm256_mask_permutexvar_ps(src, mask, arch::_mm256_set1_epi32(I as i32), value) }
    }

    fn broadcast_z<const I: usize>(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_permutexvar_ps(mask, arch::_mm256_set1_epi32(I as i32), value) }
    }

    fn broadcastv(value: Storage<Self>, idx: usize) -> Storage<Self> {
        unsafe { arch::_mm256_permutexvar_ps(arch::_mm256_set1_epi32(idx as i32), value) }
    }

    fn broadcastv_c(mask: Storage<Self::Mask>, value: Storage<Self>, idx: usize) -> Storage<Self> {
        unsafe { arch::_mm256_mask_permutexvar_ps(value, mask, arch::_mm256_set1_epi32(idx as i32), value) }
    }

    fn broadcastv_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>, idx: usize) -> Storage<Self> {
        unsafe { arch::_mm256_mask_permutexvar_ps(src, mask, arch::_mm256_set1_epi32(idx as i32), value) }
    }

    fn broadcastv_z(mask: Storage<Self::Mask>, value: Storage<Self>, idx: usize) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_permutexvar_ps(mask, arch::_mm256_set1_epi32(idx as i32), value) }
    }

    fn reverse_c(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe {
            let idx = arch::_mm256_setr_epi32(7, 6, 5, 4, 3, 2, 1, 0);
            arch::_mm256_mask_permutexvar_ps(value, mask, idx, value)
        }
    }
    ...
    fn reverse_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe {
            let idx = arch::_mm256_setr_epi32(7, 6, 5, 4, 3, 2, 1, 0);
            arch::_mm256_mask_permutexvar_ps(src, mask, idx, value)
        }
    }
    ...
    fn reverse_z(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe {
            let idx = arch::_mm256_setr_epi32(7, 6, 5, 4, 3, 2, 1, 0);
            arch::_mm256_maskz_permutexvar_ps(mask, idx, value)
        }
    }

    // vpshufb is masked per BYTE: shuffle, then one merge move.
    masked_via_mov_v4! {
        mov = _mm256_mask_mov_ps, movz = _mm256_maskz_mov_ps;
    ...
    fn permutev_m(
        ...
        unsafe { arch::_mm256_mask_permutexvar_ps(src, mask, idxs, value) }
    }

    fn permutev_z(mask: Storage<Self::Mask>, value: Storage<Self>, idxs: Storage<Self::Unsigned>) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_permutexvar_ps(mask, idxs, value) }
    }

    // vpermt2ps merges only into `a`: permute, then one merge move. The
    // zeroing form is native.
    fn swizzle_m(
        src: Storage<Self>,
        mask: Storage<Self::Mask>,
        a: Storage<Self>,
        ...
        unsafe { arch::_mm256_mask_mov_ps(src, mask, arch::_mm256_permutex2var_ps(a, idxs, b)) }
    }
    ...
    fn swizzle_z(
        ...
        unsafe { arch::_mm256_maskz_permutex2var_ps(mask, a, idxs, b) }
    }
}

#[thermite_macros::inline_always]
impl InterleaveRegister for F32x8V4 {
    fn interleave(a: Storage<Self>, b: Storage<Self>) -> (Storage<Self>, Storage<Self>) {
        ...
        unsafe {
            ...
            (
                arch::_mm256_permutex2var_ps(a, lo, b),
                arch::_mm256_permutex2var_ps(a, hi, b),
            )
        }
    }
    ...
    fn deinterleave(a: Storage<Self>, b: Storage<Self>) -> (Storage<Self>, Storage<Self>) {
        unsafe {
            ...
            (
                arch::_mm256_permutex2var_ps(a, even, b),
                arch::_mm256_permutex2var_ps(a, odd, b),
            )
        }
    }
}
...
#[thermite_macros::inline_always]
impl IndexableRegister<super::U32x8V4> for F32x8V4 {
    ...
    unsafe fn gather_m(
        ...
        unsafe { arch::_mm256_mmask_i32gather_ps::<4>(src, mask, indices, ptr) }
    }

    unsafe fn scatter(value: Storage<Self>, ptr: *mut Self::Element, indices: Storage<super::U32x8V4>) {
        unsafe { arch::_mm256_i32scatter_ps::<4>(ptr, indices, value) }
    }
    ...
    unsafe fn scatter_m(
        ...
        unsafe { arch::_mm256_mask_i32scatter_ps::<4>(ptr, mask, indices, value) }
    }
}

// Eight 64-bit indices are one zmm (`u64x8`): the qword-index gathers land
// their eight f32 results straight in a ymm.
#[thermite_macros::inline_always]
impl IndexableRegister<super::U64x8V4> for F32x8V4 {
    unsafe fn gather(ptr: *const Self::Element, indices: Storage<super::U64x8V4>) -> Storage<Self> {
        unsafe { arch::_mm512_i64gather_ps::<4>(indices, ptr) }
    }
    ...
    unsafe fn gather_m(
        ...
        unsafe { arch::_mm512_mask_i64gather_ps::<4>(src, mask, indices, ptr) }
    }

    unsafe fn scatter(value: Storage<Self>, ptr: *mut Self::Element, indices: Storage<super::U64x8V4>) {
        unsafe { arch::_mm512_i64scatter_ps::<4>(ptr, indices, value) }
    }
    ...
    unsafe fn scatter_m(
        ...
        unsafe { arch::_mm512_mask_i64scatter_ps::<4>(ptr, mask, indices, value) }
    }
}

#[thermite_macros::inline_always]
impl ShuffleRegister for F32x8V4 {
...
#[thermite_macros::inline_always]
impl PartialOrdRegister for F32x8V4 {
    fn lt(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self::Mask> {
        unsafe { arch::_mm256_cmp_ps_mask(lhs, rhs, arch::_CMP_LT_OQ) }
    }

    fn le(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self::Mask> {
        unsafe { arch::_mm256_cmp_ps_mask(lhs, rhs, arch::_CMP_LE_OQ) }
    }

    fn gt(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self::Mask> {
        unsafe { arch::_mm256_cmp_ps_mask(lhs, rhs, arch::_CMP_GT_OQ) }
    }

    fn ge(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self::Mask> {
        unsafe { arch::_mm256_cmp_ps_mask(lhs, rhs, arch::_CMP_GE_OQ) }
    }

    fn eq(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self::Mask> {
        unsafe { arch::_mm256_cmp_ps_mask(lhs, rhs, arch::_CMP_EQ_OQ) }
    }

    fn ne(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self::Mask> {
        unsafe { arch::_mm256_cmp_ps_mask(lhs, rhs, arch::_CMP_NEQ_UQ) }
    }
}

#[thermite_macros::inline_always]
impl NumericRegister for F32x8V4 {
    ...
    // No 256-bit reduce sequences in core::arch, so the v3 ymm -> xmm fold.
    ...
    masked_binary_v4! {
        add => _mm256_mask_add_ps, _mm256_maskz_add_ps;
        sub => _mm256_mask_sub_ps, _mm256_maskz_sub_ps;
        mul => _mm256_mask_mul_ps, _mm256_maskz_mul_ps;
        div => _mm256_mask_div_ps, _mm256_maskz_div_ps;
    }
    ...
    fn scale_m(
        ...
        unsafe { arch::_mm256_mask_mul_ps(src, mask, value, Self::splat(scalar)) }
    }

    fn scale_z(mask: Storage<Self::Mask>, value: Storage<Self>, scalar: Self::Element) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_mul_ps(mask, value, Self::splat(scalar)) }
    }
}

#[thermite_macros::inline_always]
impl SignedRegister for F32x8V4 {
    ...
    fn neg_c(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_xor_ps(value, mask, value, Self::NEG_ZERO) }
    }

    fn neg_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_xor_ps(src, mask, value, Self::NEG_ZERO) }
    }

    fn neg_z(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_xor_ps(mask, value, Self::NEG_ZERO) }
    }

    fn abs_c(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_andnot_ps(value, mask, Self::NEG_ZERO, value) }
    }

    fn abs_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_andnot_ps(src, mask, Self::NEG_ZERO, value) }
    }

    fn abs_z(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_andnot_ps(mask, Self::NEG_ZERO, value) }
    }

    // imm 0xD8 = `c ? b : a`, merging into A = lhs.
    fn copysign_c(mask: Storage<Self::Mask>, lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self> {
        unsafe {
            arch::_mm256_castsi256_ps(arch::_mm256_mask_ternarylogic_epi32::<
                { crate::ternlog_imm!(C & B | !C & A) },
            >(
                arch::_mm256_castps_si256(lhs),
                mask,
                arch::_mm256_castps_si256(rhs),
                arch::_mm256_castps_si256(Self::NEG_ZERO),
            ))
        }
    }
    ...
    fn copysign_m(
        ...
        unsafe { arch::_mm256_mask_mov_ps(src, mask, Self::copysign(lhs, rhs)) }
    }
...
#[thermite_macros::inline_always]
impl FloatRegister for F32x8V4 {
    const HAS_NATIVE_FMA: tribool::Tribool = tribool::True;

    type Bits = super::U32x8V4;
    type SignedBits = super::I32x8V4;
    type ExtendedPrecision = super::F64x8V4;
    ...
    // vaddsubps exists at 256-bit (AVX), so keep it. The masked forms below
    // use the xor + masked-add lowering since addsub has no EVEX encoding.
    ...
    fn rsqrt(value: Storage<Self>) -> Storage<Self> {
        cfg_select! {
            ...
            _ => unsafe { arch::_mm256_rsqrt14_ps(value) }
        }
    }
    ...
    fn rcp(value: Storage<Self>) -> Storage<Self> {
        cfg_select! {
            ...
            _ => unsafe { arch::_mm256_rcp14_ps(value) }
        }
    }

    // vrcp14/vrsqrt14 (VL): 2^-14 relative error, vs the 12-bit AVX
    // estimates v3's f32x8 uses.
    ...
    fn floor(value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_roundscale_ps(value, 0x09) }
    }

    fn ceil(value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_roundscale_ps(value, 0x0A) }
    }

    fn round(value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_roundscale_ps(value, 0x08) }
    }

    fn trunc(value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_roundscale_ps(value, 0x0B) }
    }

    fn next_up(value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_nextupps_v4(value) }
    }

    fn next_down(value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_nextdownps_v4(value) }
    }
    ...
    masked_fma_v4! {
        mov = _mm256_mask_mov_ps;
        mul_add => _mm256_mask_fmadd_ps, _mm256_maskz_fmadd_ps;
        mul_sub => _mm256_mask_fmsub_ps, _mm256_maskz_fmsub_ps;
        nmul_add => _mm256_mask_fnmadd_ps, _mm256_maskz_fnmadd_ps;
        nmul_sub => _mm256_mask_fnmsub_ps, _mm256_maskz_fnmsub_ps;
        mul_adde => _mm256_mask_fmadd_ps, _mm256_maskz_fmadd_ps;
        mul_sube => _mm256_mask_fmsub_ps, _mm256_maskz_fmsub_ps;
        nmul_adde => _mm256_mask_fnmadd_ps, _mm256_maskz_fnmadd_ps;
        nmul_sube => _mm256_mask_fnmsub_ps, _mm256_maskz_fnmsub_ps;
        fmaddsub => _mm256_mask_fmaddsub_ps, _mm256_maskz_fmaddsub_ps;
        fmsubadd => _mm256_mask_fmsubadd_ps, _mm256_maskz_fmsubadd_ps;
    }

    fn addsub_c(mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_add_ps(a, mask, a, Self::bitxor(b, Self::ALT_NEG)) }
    }

    fn addsub_m(src: Storage<Self>, mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_add_ps(src, mask, a, Self::bitxor(b, Self::ALT_NEG)) }
    }

    fn addsub_z(mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_add_ps(mask, a, Self::bitxor(b, Self::ALT_NEG)) }
    }

    masked_unary_v4! {
        sqrt => _mm256_mask_sqrt_ps, _mm256_maskz_sqrt_ps;
        floor => _mm256_mask_roundscale_ps, _mm256_maskz_roundscale_ps, 0x09;
        ceil => _mm256_mask_roundscale_ps, _mm256_maskz_roundscale_ps, 0x0A;
        round => _mm256_mask_roundscale_ps, _mm256_maskz_roundscale_ps, 0x08;
        trunc => _mm256_mask_roundscale_ps, _mm256_maskz_roundscale_ps, 0x0B;
    }

    cfg_select! {
        feature = "strict_ieee754" => {
            fn rcp_c(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
                unsafe { arch::_mm256_mask_div_ps(value, mask, Self::ONE, value) }
            }

            fn rcp_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
                unsafe { arch::_mm256_mask_div_ps(src, mask, Self::ONE, value) }
            }

            fn rcp_z(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
                unsafe { arch::_mm256_maskz_div_ps(mask, Self::ONE, value) }
            }

            fn rsqrt_c(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
                unsafe { arch::_mm256_mask_div_ps(value, mask, Self::ONE, Self::sqrt(value)) }
            }

            fn rsqrt_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
                unsafe { arch::_mm256_mask_div_ps(src, mask, Self::ONE, Self::sqrt(value)) }
            }

            fn rsqrt_z(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
                unsafe { arch::_mm256_maskz_div_ps(mask, Self::ONE, Self::sqrt(value)) }
            }
        }
        _ => {
            masked_unary_v4! {
                rcp => _mm256_mask_rcp14_ps, _mm256_maskz_rcp14_ps;
                rsqrt => _mm256_mask_rsqrt14_ps, _mm256_maskz_rsqrt14_ps;
            }
        }
    }

    fn fract_c(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_sub_ps(value, mask, value, Self::trunc(value)) }
    }

    fn fract_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_sub_ps(src, mask, value, Self::trunc(value)) }
    }

    fn fract_z(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_sub_ps(mask, value, Self::trunc(value)) }
    }
    ...
    fn mul_sign_m(
        ...
        unsafe { arch::_mm256_mask_mov_ps(src, mask, Self::mul_sign(value, sign)) }
    }
    ...
    fn signed_zero_c(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_and_ps(value, mask, Self::NEG_ZERO, value) }
    }

    fn signed_zero_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_mask_and_ps(src, mask, Self::NEG_ZERO, value) }
    }

    fn signed_zero_z(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm256_maskz_and_ps(mask, Self::NEG_ZERO, value) }
    }
...
// ExtendedPrecision legs: f32x8 (ymm) <-> f64x8 (zmm), one convert each way.
#[thermite_macros::inline_always]
impl CastRegister<F32x8V4> for super::F64x8V4 {
    fn cast_from(value: Storage<F32x8V4>) -> Storage<Self> {
        unsafe { arch::_mm512_cvtps_pd(value) }
    }
}

#[thermite_macros::inline_always]
impl CastRegister<super::F64x8V4> for F32x8V4 {
    fn cast_from(value: Storage<super::F64x8V4>) -> Storage<Self> {
        unsafe { arch::_mm512_cvtpd_ps(value) }
    }
}
