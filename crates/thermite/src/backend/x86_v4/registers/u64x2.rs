//! `u64x2` on AVX-512: the 128-bit unsigned-qword register under EVEX
//! (AVX512VL). Width port of [`u64x4`](super::u64x4). See
//! [`i64x2`](super::i64x2). The 2D Morton code uses the legacy 128-bit
//! `pclmulqdq`, which is floor for this backend (every AVX-512 part has it).
...
impl BitwiseRegister for U64x2V4 {
...
#[thermite_macros::inline_always]
impl crate::register::ConcatRegister<u64> for U64x2V4 {
    fn concat(lo: Storage<u64>, hi: Storage<u64>) -> Storage<Self> {
        unsafe { arch::_mm_set_epi64x(hi as i64, lo as i64) }
    }

    fn split(value: Storage<Self>) -> (Storage<u64>, Storage<u64>) {
        let lo = unsafe { arch::_mm_cvtsi128_si64(value) as u64 };
        let hi = unsafe { arch::_mm_extract_epi64::<1>(value) as u64 };
        (lo, hi)
    }
}

#[thermite_macros::inline_always]
impl crate::register::ExtendRegister<u64> for U64x2V4 {
    fn extend(value: Storage<u64>) -> Storage<Self> {
        unsafe { arch::_mm_cvtsi64x_si128(value as i64) }
    }
...
#[thermite_macros::inline_always]
impl crate::register::WideRegister for U64x2V4 {
    type Wide = super::U64x4V4;
}

#[thermite_macros::inline_always]
impl Register for U64x2V4 {
    type Element = u64;

    type Signed = super::I64x2V4;
    type Unsigned = U64x2V4;
    ...
    fn single(value: Self::Element) -> Storage<Self> {
        unsafe { arch::_mm_set_epi64x(0, value as i64) }
    }
    ...
    unsafe fn load_m(src: Storage<Self>, mask: Storage<Self::Mask>, ptr: *const Self::Element) -> Storage<Self> {
        unsafe { arch::_mm_mask_loadu_epi64(src, mask, ptr as *const _) }
    }

    unsafe fn load_z(mask: Storage<Self::Mask>, ptr: *const Self::Element) -> Storage<Self> {
        unsafe { arch::_mm_maskz_loadu_epi64(mask, ptr as *const _) }
    }
    ...
    unsafe fn store_masked(ptr: *mut Self::Element, mask: Storage<Self::Mask>, value: Storage<Self>) {
        unsafe { arch::_mm_mask_storeu_epi64(ptr as *mut _, mask, value) }
    }
...
#[thermite_macros::inline_always]
impl InterleaveRegister for U64x2V4 {
...
#[thermite_macros::inline_always]
impl IndexableRegister<U64x2V4> for U64x2V4 {
    unsafe fn gather(ptr: *const Self::Element, indices: Storage<U64x2V4>) -> Storage<Self> {
        unsafe { arch::_mm_i64gather_epi64::<8>(ptr as _, indices) }
    }
    ...
    unsafe fn gather_m(
        ...
        indices: Storage<U64x2V4>,
    ) -> Storage<Self> {
        unsafe { arch::_mm_mmask_i64gather_epi64::<8>(src, mask, indices, ptr as _) }
    }

    unsafe fn scatter(value: Storage<Self>, ptr: *mut Self::Element, indices: Storage<U64x2V4>) {
        unsafe { arch::_mm_i64scatter_epi64::<8>(ptr as _, indices, value) }
    }
    ...
    unsafe fn scatter_m(
        ...
        unsafe { arch::_mm_mask_i64scatter_epi64::<8>(ptr as _, mask, indices, value) }
    }
}

#[thermite_macros::inline_always]
impl BitshiftRegister for U64x2V4 {
...
impl NumericRegister for U64x2V4 {
    ...
    fn min_element(value: Storage<Self>) -> Self::Element {
        _mm_reduce_epi64_v1!(value; _mm_min_epu64) as u64
    }

    fn max_element(value: Storage<Self>) -> Self::Element {
        _mm_reduce_epi64_v1!(value; _mm_max_epu64) as u64
    }

    fn sum_elements(value: Storage<Self>) -> Self::Element {
        _mm_reduce_epi64_v1!(value; _mm_add_epi64) as u64
    }

    fn prod_elements(value: Storage<Self>) -> Self::Element {
        _mm_reduce_epi64_v1!(value; _mm_mullo_epi64) as u64
    }
    ...
    fn min(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_min_epu64(lhs, rhs) }
    }

    fn max(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_max_epu64(lhs, rhs) }
    }
    ...
    masked_binary_v4! {
        ...
        min => _mm_mask_min_epu64, _mm_maskz_min_epu64;
        max => _mm_mask_max_epu64, _mm_maskz_max_epu64;
    }
...
#[thermite_macros::inline_always]
impl IntegerRegister for U64x2V4 {
    fn mulhi(lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self> {
        unsafe {
            let lomask = arch::_mm_set1_epi64x(0xFFFF_FFFF);
            let xh = arch::_mm_shuffle_epi32::<0b10_11_00_01>(lhs);
            let yh = arch::_mm_shuffle_epi32::<0b10_11_00_01>(rhs);
            ...
            let w0h = arch::_mm_srli_epi64::<32>(w0);
            let s1 = arch::_mm_add_epi64(w1, w0h);
            let s1l = arch::_mm_and_si128(s1, lomask);
            let s1h = arch::_mm_srli_epi64::<32>(s1);
            let s2 = arch::_mm_add_epi64(w2, s1l);
            let s2h = arch::_mm_srli_epi64::<32>(s2);

            arch::_mm_add_epi64(arch::_mm_add_epi64(w3, s1h), s2h)
        }
    }
    ...
    fn saturating_add_c(mask: Storage<Self::Mask>, lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_mask_add_epi64(lhs, mask, lhs, Self::min(rhs, Self::not(lhs))) }
    }
    ...
    fn saturating_add_m(
        ...
        unsafe { arch::_mm_mask_add_epi64(src, mask, lhs, Self::min(rhs, Self::not(lhs))) }
    }

    fn saturating_add_z(mask: Storage<Self::Mask>, lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_maskz_add_epi64(mask, lhs, Self::min(rhs, Self::not(lhs))) }
    }

    fn saturating_sub_c(mask: Storage<Self::Mask>, lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_mask_sub_epi64(lhs, mask, Self::max(lhs, rhs), rhs) }
    }
    ...
    fn saturating_sub_m(
        ...
        unsafe { arch::_mm_mask_sub_epi64(src, mask, Self::max(lhs, rhs), rhs) }
    }

    fn saturating_sub_z(mask: Storage<Self::Mask>, lhs: Storage<Self>, rhs: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_maskz_sub_epi64(mask, Self::max(lhs, rhs), rhs) }
    }
...
#[thermite_macros::inline_always]
impl UnsignedIntegerRegister for U64x2V4 {
    ...
    /// 2D Morton via the 128-bit carry-less multiply (`pclmulqdq`, floor:
    /// every AVX-512 part has it). Every other `N` uses the generic cascade.
    fn morton<const N: usize>(values: [Storage<Self>; N]) -> Storage<Self> {
        if const { N == 2 } {
            unsafe {
                let lomask = arch::_mm_set1_epi64x(0xFFFF_FFFF);

                let x = arch::_mm_and_si128(values[0], lomask);
                let y = arch::_mm_and_si128(values[1], lomask);

                // Self clmul spreads the low 32 bits of a qword across 64.
                // Imm 0x00/0x11 pick the low/high qword, unpacklo re-packs.
                let xs = arch::_mm_unpacklo_epi64(
                    arch::_mm_clmulepi64_si128::<0x00>(x, x),
                    arch::_mm_clmulepi64_si128::<0x11>(x, x),
                );
                let ys = arch::_mm_unpacklo_epi64(
                    arch::_mm_clmulepi64_si128::<0x00>(y, y),
                    arch::_mm_clmulepi64_si128::<0x11>(y, y),
                );

                arch::_mm_or_si128(xs, arch::_mm_slli_epi64::<1>(ys))
            }
        } else {
    ...
    fn ilog2p1_c(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_mask_sub_epi64(value, mask, Self::splat(64), Self::leading_zeros(value)) }
    }

    fn ilog2p1_m(src: Storage<Self>, mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_mask_sub_epi64(src, mask, Self::splat(64), Self::leading_zeros(value)) }
    }

    fn ilog2p1_z(mask: Storage<Self::Mask>, value: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_maskz_sub_epi64(mask, Self::splat(64), Self::leading_zeros(value)) }
    }

    fn avg_c(mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_mask_sub_epi64(a, mask, Self::bitor(a, b), Self::shri::<1>(Self::bitxor(a, b))) }
    }

    fn avg_m(src: Storage<Self>, mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_mask_sub_epi64(src, mask, Self::bitor(a, b), Self::shri::<1>(Self::bitxor(a, b))) }
    }

    fn avg_z(mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_maskz_sub_epi64(mask, Self::bitor(a, b), Self::shri::<1>(Self::bitxor(a, b))) }
    }

    fn abs_diff_c(mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_mask_sub_epi64(a, mask, Self::max(a, b), Self::min(a, b)) }
    }

    fn abs_diff_m(src: Storage<Self>, mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_mask_sub_epi64(src, mask, Self::max(a, b), Self::min(a, b)) }
    }

    fn abs_diff_z(mask: Storage<Self::Mask>, a: Storage<Self>, b: Storage<Self>) -> Storage<Self> {
        unsafe { arch::_mm_maskz_sub_epi64(mask, Self::max(a, b), Self::min(a, b)) }
    }
