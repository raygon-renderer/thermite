...
impl ConcatRegister<f64> for F64x2V2 {
...
#[thermite_macros::inline_always]
impl InterleaveRegister for F64x2V2 {
...
impl Register for F64x2V2 {
    ...
    impl_float_align_via_bits!(super::U64x2V2);

    fn permutev(value: Storage<Self>, idxs: Storage<Self::Unsigned>) -> Storage<Self> {
        // As `I64x2V2::permutev`, through the pd wrapper.
...
impl FloatRegister for F64x2V2 {
    const HAS_NATIVE_FMA: tribool::Tribool = tribool::False;

    type Bits = super::U64x2V2;
    type SignedBits = super::I64x2V2;
    ...
    // SSE3 native alternating add/sub; no packed FMA, so the fused variants are a
    // plain multiply then the native addsub.
