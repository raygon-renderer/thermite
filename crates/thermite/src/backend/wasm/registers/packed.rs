//! `PackedFloatRegister` for the native 16-bit registers on the WASM SIMD128 backend.
//!
//! WASM has no F16C-equivalent, so every 16-bit-float format uses the generic branchless
...
use super::half16::U16x4Wasm;
use super::{F32x4Wasm, U16x8Wasm};

/// f32 partner per `u16` width - the backend's `Simd::f32xK` (native f32 is 4-wide on WASM, so the
...
impl_packed_f16!(U16x4Wasm, F32x4);
impl_packed_f16!(U16x8Wasm, F32x8);
impl_packed_f16!(ArrayRegister<U16x8Wasm, 2>, F32x16);
