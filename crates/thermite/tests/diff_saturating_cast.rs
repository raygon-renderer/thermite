...
macro_rules! sat_diff {
    ($label:expr, $b:ty, $src:ident, $dst:ident, $se:ty) => {{
        ...
        for input in harness::corpus::<$se>(lanes, &mut rng) {
            let got = harness::read::<<$b as Simd>::$dst>(
                &<<$b as Simd>::$dst as SaturatingCastRegister<<$b as Simd>::$src>>::saturating_cast_from(
                    harness::make_array::<<$b as Simd>::$src>(&input),
                ),
            );
            let want = harness::read::<<Scalar as Simd>::$dst>(
                &<<Scalar as Simd>::$dst as SaturatingCastRegister<<Scalar as Simd>::$src>>::saturating_cast_from(
                    harness::make_array::<<Scalar as Simd>::$src>(&input),
                ),
            );
            harness::assert_lanes_eq(concat!($label, " [sat cast vs scalar clamp]"), &[], &got, &want, Tol::Exact);
        }
    }};
}
