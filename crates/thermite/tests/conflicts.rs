...
//! lowering differs per backend (and AVX-512 has `vpconflictd/q`), so
//! cross-backend agreement is the point.
#![cfg(any(
    target_arch = "x86",
    target_arch = "x86_64",
    target_arch = "wasm32",
...
/// scalar oracles.
macro_rules! check {
    ($vty:ty, $elem:ty, $n:literal, $alphabet:literal) => {{
        type V = $vty;
        const N: usize = $n;
        const A: usize = $alphabet;

        // No per-lane accessor on `Mask`, so materialize through `select`.
        let set_at = |m: <V as GenericVector>::Mask, i: usize| m.select(V::ONE, V::ZERO).as_slice()[i] != 0 as $elem;
...
for_each_backend_concrete! {
    fn i32x4() { check!(thermite::simd::i32x4<S>, i32, 4, 4) }
    fn u32x4() { check!(thermite::simd::u32x4<S>, u32, 4, 4) }
    fn i32x8() { check!(thermite::simd::i32x8<S>, i32, 8, 3) }
    fn u32x16() { check!(thermite::simd::u32x16<S>, u32, 16, 2) }
    fn u16x8() { check!(thermite::simd::u16x8<S>, u16, 8, 3) }
    fn i64x4() { check!(thermite::simd::i64x4<S>, i64, 4, 4) }
    fn u64x2() { check!(thermite::simd::u64x2<S>, u64, 2, 4) }
    fn u64x8() { check!(thermite::simd::u64x8<S>, u64, 8, 3) }
    fn i16x16() { check!(thermite::simd::i16x16<S>, i16, 16, 2) }
    fn u8x16() { check!(thermite::simd::u8x16<S>, u8, 16, 2) }
