...
//!
//! These impls are generic over `R`, so one instantiation per impl covers its
//! source lines. Running the same body on every backend additionally covers the
//! per-backend masked lowerings (`_c`/`_m`/`_z` differ between the x86 blend
//! path, the AVX-512 opmask path and the scalar select).
#![cfg(any(
    target_arch = "x86",
    target_arch = "x86_64",
    target_arch = "wasm32",
...
/// Per-backend context, emitted as items at the top of each test body.
macro_rules! ctx {
    () => {
        #[allow(dead_code)]
        type VI = Vector<<S as Simd>::i32x4>;
        #[allow(dead_code)]
        type VF = Vector<<S as Simd>::f32x4>;
    };
}
...
for_each_backend_concrete! {
    fn utility_and_accessors() {
        ...
        let reg: Storage<<S as Simd>::i32x4> = v.into_register();
    ...
    fn unsigned_predicates_dividers_fastcast() {
        ctx!();
        type VU = Vector<<S as Simd>::u32x4>;

        // is_power_of_two (UnsignedIntegerVector)
        let u = VU::new([1, 3, 8, 0]);
        let pot = u.is_power_of_two().select(VU::ONE, VU::ZERO).into_array();
        // 1 and 8 are powers of two, while 3 and 0 are not (0 matches u32::is_power_of_two)
        assert_eq!(pot.as_slice(), &[1, 0, 1, 0]);
    ...
    fn square_masked() {
        ...
        check::<<S as Simd>::i32x4>();
        check::<<S as Simd>::f32x4>();
        check::<<S as Simd>::i32x8>();
        check::<<S as Simd>::f64x8>();
    ...
    fn mask_reshape_and_store_masked() {
        ctx!();
        type VI8 = Vector<<S as Simd>::i32x8>;
        type MI8 = Mask<<S as Simd>::i32x8>;

        let lo = VI::new([1, 0, 1, 0]).cmp_ne(VI::ZERO); // [T,F,T,F]
        let hi = VI::new([0, 0, 1, 1]).cmp_ne(VI::ZERO); // [F,F,T,T]
        let rb4 = |m: Mask<<S as Simd>::i32x4>| m.select(VI::ONE, VI::ZERO).into_array().as_slice().to_vec();
