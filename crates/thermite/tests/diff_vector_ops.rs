...
//! `vector/ops.rs`. These are thin wrappers over the register ops,
...
//! Runs on every backend at x4, x8 and x16 (native or emulated).
#![cfg(any(
    target_arch = "x86",
    target_arch = "x86_64",
    target_arch = "wasm32",
...
macro_rules! float_ops {
    ($freg:ident) => {{
        ...
        un!(fpos, fsrc, F::ZERO, mf, &mbf, rd(fpos.sqrt()), sqrt_c, sqrt_m, sqrt_z);
        ...
        un!(fa, fsrc, F::ZERO, mf, &mbf, rd(fa.floor()), floor_c, floor_m, floor_z);
        un!(fa, fsrc, F::ZERO, mf, &mbf, rd(fa.ceil()), ceil_c, ceil_m, ceil_z);
        un!(fa, fsrc, F::ZERO, mf, &mbf, rd(fa.round()), round_c, round_m, round_z);
        un!(fa, fsrc, F::ZERO, mf, &mbf, rd(fa.trunc()), trunc_c, trunc_m, trunc_z);
        un!(fa, fsrc, F::ZERO, mf, &mbf, rd(fa.fract()), fract_c, fract_m, fract_z);
...
for_each_backend_concrete! {
    fn float_ops_x4() { float_ops!(f32x4) }
    fn float_ops_x8() { float_ops!(f32x8) }
    fn float_ops_x16() { float_ops!(f32x16) }
    fn int_ops_x4() { int_ops!(i32x4) }
    fn int_ops_x8() { int_ops!(i32x8) }
    fn int_ops_x16() { int_ops!(i32x16) }
}
