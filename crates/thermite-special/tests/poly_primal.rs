//! `poly_n_primal`/`poly_rev_n_primal` against `poly`/`poly_rev` across the composite types.
//!
...
//! of a constant (which are zero). `poly_n_primal` carries them in `Self::Primal` and
...
#[test]
fn poly_n_primal_matches_poly_on_real_vectors() {
    for x in [-2.0f64, -0.5, 0.0, 0.25, 1.0, 3.5] {
        let a = V::splat(x).poly_n(&COEFFS).extract::<0>();
        let b = V::splat(x).poly_n_primal(&splat_primal::<V>(COEFFS)).extract::<0>();
        assert_eq!(a, b, "real poly at {x}: poly {a}, poly_n_primal {b}");

        let a = V::splat(x).poly_rev_n(&COEFFS).extract::<0>();
        let b = V::splat(x).poly_rev_n_primal(&splat_primal::<V>(COEFFS)).extract::<0>();
        assert_eq!(a, b, "real poly_rev at {x}: poly_rev {a}, poly_rev_n_primal {b}");
    }
}

#[test]
fn poly_n_primal_matches_poly_on_dual() {
    type D = Dual<V, 2>;
    ...
    for x in [-2.0f64, -0.5, 0.25, 1.0, 3.5] {
        ...
        let a = d.poly_n(&lifted);
        let b = d.poly_n_primal(&splat_primal::<D>(COEFFS));
        ...
        let a = d.poly_rev_n(&lifted);
        let b = d.poly_rev_n_primal(&splat_primal::<D>(COEFFS));
...
#[test]
fn poly_n_primal_matches_poly_on_complex() {
    type C = Complex<V>;
    ...
    for (re, im) in [(0.5f64, 0.25f64), (-1.5, 2.0), (0.0, -0.75), (3.0, 0.0)] {
        ...
        let a = z.poly_n(&lifted);
        let b = z.poly_n_primal(&splat_primal::<C>(COEFFS));

        assert_eq!(a.re.extract::<0>(), b.re.extract::<0>(), "complex re at ({re},{im})");
        assert_eq!(a.im.extract::<0>(), b.im.extract::<0>(), "complex im at ({re},{im})");

        let a = z.poly_rev_n(&lifted);
        let b = z.poly_rev_n_primal(&splat_primal::<C>(COEFFS));
...
#[test]
fn poly_n_primal_matches_poly_on_compensated() {
    ...
    // `Compensated`, and `poly_n_primal` must reduce to `poly` exactly.
    ...
    for x in [-2.0f64, 0.25, 1.0, 3.5] {
        ...
        let a = c.poly_n(&lifted).value().extract::<0>();
        let b = c.poly_n_primal(&splatted).value().extract::<0>();

        assert_eq!(a, b, "compensated poly at {x}: poly {a}, poly_n_primal {b}");

        let a = c.poly_rev_n(&lifted).value().extract::<0>();
        let b = c.poly_rev_n_primal(&splatted).value().extract::<0>();

        assert_eq!(a, b, "compensated poly_rev at {x}: poly_rev {a}, poly_rev_n_primal {b}");
    }
}
...
/// takes the Estrin/ILP path, anything else takes Horner. `poly_n_primal` overrides the
/// real-vector case specifically to keep that split rather than inherit the
/// primal-Horner default, so both arms need checking.
#[test]
fn poly_n_primal_matches_poly_under_each_policy() {
    ...
    macro_rules! check {
        ($($policy:ty),*) => {$(
            for x in [-2.0f64, -0.5, 0.0, 0.25, 1.0, 3.5] {
                let v = V::splat(x);
                let a = v.poly_n_p::<$policy, 6>(&COEFFS).extract::<0>();
                let b = v.poly_n_primal_p::<$policy, U6>(&splat_primal::<V>(COEFFS)).extract::<0>();
                assert!(
                    (a - b).abs() <= 1e-14 * (1.0 + a.abs()),
                    "{} at {x}: poly {a}, poly_n_primal {b}",
                    core::any::type_name::<$policy>()
                );

                let a = v.poly_rev_n_p::<$policy, 6>(&COEFFS).extract::<0>();
                let b = v.poly_rev_n_primal_p::<$policy, U6>(&splat_primal::<V>(COEFFS)).extract::<0>();
                assert!(
                    (a - b).abs() <= 1e-14 * (1.0 + a.abs()),
                    "{} rev at {x}: poly_rev {a}, poly_rev_n_primal {b}",
                    core::any::type_name::<$policy>()
                );
            }
        )*};
