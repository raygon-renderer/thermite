...
decl!(f64x1: f64 => f64);
impl<S: Simd> Default for f64x1<S> {
    #[inline(always)]
    fn default() -> Self {
        Self::new(0.0)
    }
...
impl SimdVectorBase<Scalar> for f64x1<Scalar> {
    type Element = f64;
    ...
    decl_base_common!(#[target_feature()] f64x1: f64 => f64);
}

impl SimdBitwise<Scalar> for f64x1<Scalar> {
    ...
    unsafe fn _mm_bitand(self, rhs: Self) -> Self {
        Self::new(f64::from_bits(self.value.to_bits() & rhs.value.to_bits()))
    }
    ...
    unsafe fn _mm_shr(self, count: Vu32) -> Self {
        Self::new(f64::from_bits(self.value.to_bits() << count.value))
    }
    ...
    unsafe fn _mm_shli(self, count: u32) -> Self {
        Self::new(f64::from_bits(self.value.to_bits() << count))
    }
...
impl PartialEq<Self> for f64x1<Scalar> {
    #[inline(always)]
...
impl SimdMask<Scalar> for f64x1<Scalar> {
    #[inline(always)]
...
impl SimdVector<Scalar> for f64x1<Scalar> {
    fn zero() -> Self {
        Self::splat(0.0)
    }
    ...
    fn eq(self, other: Self) -> Mask<Scalar, Self> {
        Self::new(f64::from_bits(bool_to_u32(self.value == other.value)))
    }
...
impl SimdIntoBits<Scalar, Vu32> for f64x1<Scalar> {
    fn into_bits(self) -> Vu32 {
        u32x1::new(self.value.to_bits())
    }
}

impl SimdFromBits<Scalar, Vu32> for f64x1<Scalar> {
    fn from_bits(bits: Vu32) -> Self {
        Self::new(f64::from_bits(bits.value))
    }
}

impl_ops!(@UNARY f64x1 Scalar => Not::not, Neg::neg);
impl_ops!(@BINARY f64x1 Scalar => Add::add, Sub::sub, Mul::mul, Div::div, Rem::rem, BitAnd::bitand, BitOr::bitor, BitXor::bitxor);
impl_ops!(@SHIFTS f64x1 Scalar => Shr::shr, Shl::shl);
